...
public class GameExpertActivity extends ActionBarActivity implements View.OnClickListener {
    ...
    private GameUtils.GAME_DIFFICULT game_difficult = GameUtils.GAME_DIFFICULT.EXPERT;
    ...
    private ImageButton btnRed, btnYellow, btnBlue, btnGreen, btnOrange, btnPink;
    ...
    private int soundRedId, soundYellowId, soundBlueId, soundGreenId, soundOrangeId, soundPinkId;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btnGreen:
                ...
                if (isGameRunning)
                    if (levelsSequence[levelCarret] == GameUtils.GAME_COLORS.GREEN) {
                        if (++levelCarret == currentLevel)
                            notifySuccess();
                    } else {
                        notifyFailure();
            ...
            case R.id.btnOrange:
                ...
                if (isGameRunning)
                    if (levelsSequence[levelCarret] == GameUtils.GAME_COLORS.ORANGE) {
                        if (++levelCarret == currentLevel)
                            notifySuccess();
                    } else {
                        notifyFailure();
            ...
            case R.id.btnPink:
                ...
                if (isGameRunning)
                    if (levelsSequence[levelCarret] == GameUtils.GAME_COLORS.PINK) {
                        if (++levelCarret == currentLevel)
                            notifySuccess();
                    } else {
                        notifyFailure();
